...
__global__ void loadImage(uchar3* srcImage,
                          float3* dstImage,
                          uint32_t width,
                          uint32_t height)
{
...
//	printf("cuda thread %i %i  %i %i pixel %i \n", x, y, width, height, pixel);
    ...
    dstImage[pixel] = make_float3( 255, 255, 255);
...
cudaError_t cudaRGBToRGBAf( uchar3* srcDev, float3* destDev, size_t width, size_t height )
{
	if( !srcDev || !destDev )
	...
	loadImage<<<gridDim, blockDim>>>( srcDev, destDev, width, height );
